...
async def test_get_claim_unauthorized():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        response = await ac.get(
            f"{settings.API_V1_STR}/claims/123e4567-e89b-12d3-a456-426614174000"
        )
    assert response.status_code == 403
...
async def test_get_policy_authorized():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        response = await ac.get(
            f"{settings.API_V1_STR}/policies/POL-123456789", headers=headers
        )
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        try:
            response = await ac.post(
                f"{settings.API_V1_STR}/claims/", json=payload, headers=headers
            )
